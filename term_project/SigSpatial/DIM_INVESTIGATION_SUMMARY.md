...
6. **Fix / path forward:** use a WIDE embedding (e.g., the 4096 layer, or train `18220→4096→4096` with no funnel). To expose the whole recall↔throughput frontier from ONE model, use **Matryoshka** (apply the WJ loss on nested prefixes {512,1024,2048,4096} of a single wide embedding → truncate to any dim at inference).
7. **Separate critical bug (root-caused earlier):** the first 2048 triplet run *collapsed* because it used `max_pos=256` (from the InfoNCE runner). Triplet's hard margin is unsatisfiable with broad positives → identical embeddings. **Triplet needs `max_pos=30`.** Fixed; the 2048 numbers are post-fix and healthy. InfoNCE *does* tolerate max_pos=256.
