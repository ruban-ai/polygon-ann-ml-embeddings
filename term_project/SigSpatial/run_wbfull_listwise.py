#!/usr/bin/env python3
"""Water-body FULL-SCALE listwise: the last generalization gap for the paper --
water-body has only ever been tested at the 50K (40k corpus/10k query) scale.
This mirrors what run_full187k_listwise.py did for Parks: train on the full
358,840-polygon corpus's internal neighbour structure (every held-out query is
genuinely unseen), then evaluate ALL 89,710 queries against the full corpus.
Same architecture/optimizer/listwise loss/tau/prefixes as every other listwise
run in this paper; EPOCHS=5 to match the full-scale Parks protocol (not the
50K protocol's 10), since a full epoch here already sees ~10.8M pairs.

Launch: torchrun --standalone --nproc_per_node=8 run_wbfull_listwise.py"""
...
EF = 200; RB = 64; CAND_KS = [1000, 2000]; SEED = 42; QS = 358840
...
CKPT = f'{BASE}/best_wbfull_listwise.pt'
...
def evaluate(embed_fn, gt, qs, Cr, Qr, Cr_sum, qtn, DEV, tag):
    ...
    note = (f"wb-real0.006 FULL-SCALE listwise, corpus-trained on {QS} corpus, "
            f"eval ALL {len(Qr)} held-out queries; raw-WJ GT; tau={TAU}; efSearch={EF}; "
            f"mirrors run_full187k_listwise.py protocol for the second polygon class")
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for m, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "wbfull_allq", f"wbfull-listwise-d{m}", m, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_wbfull_listwise.py", note])
...
def main():
    ...
    evaluate(enc.embed, gt, QS, Cr, Qr, Cr_sum, QTN, DEV, 'wbfull-listwise')
