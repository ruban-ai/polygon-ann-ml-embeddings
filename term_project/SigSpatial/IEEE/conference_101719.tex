...
needed.}\rs{Done -- rewritten below in that order (problem, impact/example,
SOTA, limitations, gap, method, contributions); internal section/equation
references removed throughout, numbers trimmed to the ones that felt load-bearing.}
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  \item \textbf{The first learned, shape-aware compression for WJ polygon retrieval.}
    \ba{Rephrase the description according to what I sent in discord.}\rs{Need the
    Discord text pasted here -- I don't have access to Discord, can't rephrase this
    one without it.}
    Unlike ShapeToVec, ShapeToVec+~\cite{shapetovecplus26}, and other exact-computation
    geometry signatures, our encoder is \emph{trained} on the ground-truth shape
    similarity structure, learning what makes two polygons look alike rather than
    encoding them by a fixed rule. Against ShapeToVec's own published
    results~\cite{shape2vec} on the identical Parks dataset, our embeddings, up to
    $47\times$ smaller, reach Recall@50 $=92$--$93\%$ without any reranking, close
    behind their $96$--$97\%$ at up to $14\times$ the throughput, and exceed their best
    published recall entirely after a cheap exact-WJ rerank pass, while remaining
    several-fold faster than their most accurate configuration.
...
this directly with bit-quantized signatures spanning $1$--$8$ bits per dimension: the
8-bit configuration is lossless, while 1-bit trades accuracy for the smallest
footprint. Both remain \emph{exact} computations over the same quadtree encoding, with
