...
\begin{abstract}
Finding polygons with similar shape in large geospatial datasets underlies map
conflation, land-use change detection, and geospatial intelligence. Shape similarity
here means area-weighted overlap independent of location, for which Weighted Jaccard
(WJ) -- the ratio between overlapping and combined area -- is the standard metric.
Real-world polygon areas vary by many orders of magnitude,
and a uniform encoding grid cannot serve both regimes: fine enough to resolve small
polygons and it explodes in dimension for large ones, coarse enough to be tractable and
small polygons collapse into near-empty, uninformative vectors. The state-of-the-art,
ShapeToVec, resolves this extreme area variability with an adaptive quadtree grid,
encoding each polygon as a high-dimensional floating-point vector
\ba{ShapeToVec does not use simplex vectors. The best performing vectors in that work
are floating-point encodings. Be mindful when using technical terms, as this was a
complaint before.}\rs{Done -- fixed.} indexed directly
under WJ with HNSW, but that high dimensionality costs heavily in memory and query
throughput.
\ba{You can also mention the bit vectors used and write about the compression
introduced by ShapeToVec+ (up to 32x) and its limitations in terms of compression
ability and reduced accuracy.}\rs{Done -- added next sentence.}
ShapeToVec+~\cite{shapetovecplus26} compresses these into compact
bit-signatures, up to $32\times$ smaller, but at real accuracy cost at its most
aggressive settings (recall falling from $96\%$ to $76\%$ on the identical dataset).
Both rely on
\emph{designed} signatures: exact, hand-engineered encodings with no learned
component. We instead introduce \emph{learned} signatures via \emph{WJ-native
distillation}: a simplex-constrained encoder that maps each polygon's raw vector onto
a compact probability simplex. It is trained with a rank-aware \emph{listwise} loss:
each anchor's true, exact-WJ-ranked neighbours
\ba{Suggestion: you can say true/exact top k similar shapes rather than explaining
what you do in your code}\rs{Done -- reworded.}
guide training, supervising the full
ranking structure rather than individual values. This beats several alternative
training objectives we test, at every scale.
\ba{I think it is too early to mention these method names etc, I got lost not
knowing what these mean}\rs{Done -- removed method names.}
A single trained encoder serves every
deployed width via Matryoshka-style truncation; one training run
\ba{replace so as it does not sound formal}\rs{Done.}
therefore covers
every operating point from 256 to 4096 dimensions. Against ShapeToVec's own
published results~\cite{shape2vec} on the identical Parks dataset (233,773 polygons),
our embeddings, up to $47\times$ smaller, reach within a few points of their best
recall without reranking, at up to $14\times$ the throughput, and exceed it entirely
after a cheap exact-WJ rerank pass while remaining several-fold faster than their most
accurate configuration.
\ba{Think: I propose defining our prior encoding attempts as designed signatures and
this as learned signatures. Does `distillation' explains what you do better?}\rs{Done
-- see designed/learned split above. Right call?}
\end{abstract}
...
\subsection{Define the problem in general}

Finding polygons with similar shape in a large geospatial dataset is a core
retrieval problem: given a query polygon, return the database polygons whose
shape overlaps it most, independent of location. Weighted Jaccard (WJ)~\cite{rajaraman11}
-- the ratio between a pair of polygons' overlapping and combined area, computed cell
by cell over a shared spatial grid -- is the standard metric for this notion of shape
similarity. The real difficulty is that real-world polygon areas vary by many orders
of magnitude, and no single, uniform grid resolution serves both regimes at once: a
grid fine enough to resolve small polygons explodes in dimension when applied to
large ones, while a grid coarse enough to stay tractable collapses small polygons
into near-empty, uninformative vectors.

\subsection{State the impact including one example.}

Finding similar shapes in large polygon datasets is a fundamental geospatial
operation, useful for deduplication, conflation, and map integration.
\ba{Too long. Please split into multiple sentences.}\rs{Done -- split into three
sentences below.}
It also underlies geospatial-intelligence tasks such as change detection. Tracking
how a lake shrinks or an ice shelf fragments over time, for example, requires
matching each new observation against its closest-shaped counterpart in a large
historical archive~\cite{shape2vec}.

\subsection{Current SOTA solutions focusing on ShapeToVec and ShapeToVec+. Also
explain other methods you mentioned in the abstract including Matryoshka.}

ShapeToVec~\cite{shape2vec} is the state of the art for this problem. It resolves
extreme area variability with an adaptive quadtree grid that recursively subdivides
space where polygon detail is concentrated, encoding each polygon as a
high-dimensional floating-point vector whose WJ preserves shape overlap across
scales. Indexed with HNSW~\cite{hnsw20} under a custom Weighted Jaccard distance, it
reaches the highest reported recall for this task. ShapeToVec+~\cite{shapetovecplus26}
builds directly on this encoding, replacing ShapeToVec's raw floating-point cell
values with compact bit-quantized signatures to shrink its memory footprint.
Separately, Matryoshka representation learning~\cite{matryoshka22} trains a single
embedding so its own leading prefixes stay useful at every truncation width, letting
one model serve many deployment sizes; it targets Euclidean or cosine similarity, not
Weighted Jaccard.

\subsection{Limitations of the SOTA methods}

ShapeToVec's accuracy comes at a real cost: its floating-point vectors are large, and
storing and searching them grows expensive as a dataset grows. ShapeToVec+ shrinks
that footprint, but only by degrading accuracy at its most aggressive settings, since
bit-quantization discards the fine detail a floating-point value carries. Both remain
\emph{designed} signatures: fixed, hand-engineered encoding rules with no learned
component, so neither can adapt its footprint to what actually matters for a given
collection's shapes. Matryoshka representation learning solves a different problem --
multi-width deployment -- and does not itself produce WJ-compatible embeddings.

\subsection{How to fill the gap. You can distinguish learned signatures vs
designed (from prior attempts)}

We close this gap with a \emph{learned} signature instead of a \emph{designed} one.
Rather than hand-engineering a fixed encoding rule, we train an encoder directly on
the ground-truth shape similarity structure, so it learns what makes two polygons
look alike and represents that information as compactly as the data allows. This is a
different lever than ShapeToVec+'s bit-quantization: instead of shrinking each
dimension's precision, we learn which dimensions to keep at all.

\subsection{The proposed method including major steps}

Our encoder maps each polygon's raw ShapeToVec vector onto a compact probability
simplex, so its output is directly compatible with Weighted Jaccard and indexes under
the same HNSW pipeline ShapeToVec already uses, with no new index structure required.
It is trained with a rank-aware listwise loss that turns the training objective into
the retrieval metric itself: each anchor's true, exact-WJ-ranked neighbours guide
training, so the compressed embedding's own ranking is taught to match the ground
truth directly, not just its magnitude. A single trained encoder serves every
deployed embedding width via Matryoshka-style truncation, so one training run covers
every operating point we deploy. At query time, a cheap exact-WJ rerank pass over the
small candidate set HNSW returns closes any remaining gap to the uncompressed
ceiling.
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  \item \ba{Rephrase the description according to what I sent in discord.}\rs{Done --
    reframed around the ShapeToVec $\to$ ShapeToVec+ $\to$ us story from Discord.}
    \textbf{Learned signatures that exceed ShapeToVec+'s compression ceiling at
    on-par or better accuracy.} ShapeToVec is the state of the art for
    weighted-Jaccard polygon retrieval, but its best-performing floating-point
    signatures are high-dimensional and do not scale. ShapeToVec+ shrinks these with
    hand-designed bit-quantization schemes, capped at $32\times$ compression with a
    real accuracy cost at that ceiling. We instead learn the signature directly from
    the ground-truth shape-similarity structure: against ShapeToVec's own published
    results~\cite{shape2vec} on the identical Parks dataset, our embeddings reach up
    to $47\times$ smaller than their tested configurations -- exceeding ShapeToVec+'s
    reported compression ceiling -- while matching their recall without reranking and
    exceeding it entirely after a cheap exact-WJ rerank pass, at several-fold their
    throughput.
  \item \ba{Write directly describe the feature you have.}\rs{Done -- reworded to lead
    with the feature.}
    ...
    absolute value. This beats absolute-value regression (MSE) and five further
    alternatives we test, decisively, at every scale tested.
  \item \ba{Present the contribution as you did in contribution 1 and then
    explain.}\rs{Done -- headline claim first, explanation after.}
    \textbf{A single learned encoder that serves every deployment width, with no
    retraining per size.} A WJ-native simplex encoder (ReLU\,+\,$\ell_1$
    output on the probability simplex, indexable directly under WJ with no
    post-processing) is deployed via Matryoshka-style truncation: one trained model
    serves every embedding width $\{256, 512, \dots,4096\}$ from a single round of
    training, and the width range itself is a training-time configuration, not a
    fixed architectural choice -- serving a narrower range needs no redesign, only
    retraining with that range. 256-d dominates the recall--throughput frontier among
    the widths we test.
\end{enumerate}
...
numbers in Table~\ref{tab:lossablation}). We further tested five rank-aware and
Matryoshka-training refinements layered on top of listwise
