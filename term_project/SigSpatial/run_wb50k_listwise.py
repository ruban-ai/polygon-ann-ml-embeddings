#!/usr/bin/env python3
"""Water-body listwise: closes the last gap in the full-paper rewrite -- every
Parks result (10K/50K/187K) has already established listwise beats MSE distillation
decisively, but the water-body benchmark (a second polygon class, built for
Review 2's ask) was only ever trained with MSE (run_wb50k_ddp.py --method distill).
Same architecture/optimizer/listwise loss/tau/epoch budget as run_50k_listwise.py,
just pointed at the water-body data (same convention as run_wb50k_ddp.py vs
run_50k_ddp.py) -- confirms listwise's advantage generalizes to the second polygon
class too, not just Parks.

Launch: torchrun --standalone --nproc_per_node=8 run_wb50k_listwise.py"""
...
CKPT = '/raid/ruban/hpmlproj/term_project/SigSpatial/best_wb50k_listwise.pt'
...
def evaluate(embed_fn, gt, qs, Cr, Qr, Cr_sum, qtn, DEV, tag):
    ...
    note = f"wb-real0.006 listwise corpus-trained, eval held-out queries; raw-WJ; tau={TAU}; efSearch={EF}; 40k corpus/10k queries; full-paper listwise-parity fill-in"
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for m, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "wb50k", f"wb50k-listwise-d{m}", m, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_wb50k_listwise.py", note])
...
def main():
    ...
    if main: print(f"DDP world={world} method=listwise corpus={QS} queries={qt.shape[0]-QS} dim={IN} tau={TAU}", flush=True)
    ...
    if not os.path.exists(KNN_CACHE):
        ...
        np.save(f'/raid/ruban/hpmlproj/term_project/SigSpatial/knn_wb50k_listwise_shard_{lr}.npy', out); dist.barrier()
        if main:
            knn = np.concatenate([np.load(f'/raid/ruban/hpmlproj/term_project/SigSpatial/knn_wb50k_listwise_shard_{r}.npy') for r in range(world)], axis=0)
    ...
    evaluate(enc.embed, gt, QS, Cr, Qr, Cr_sum, QTN, DEV, 'listwise')
