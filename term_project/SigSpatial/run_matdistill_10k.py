#!/usr/bin/env python3
"""10K WJ-native MATRYOSHKA distillation -> Table 1 (512-d row + dim story).
Same protocol as the 10K baselines (train on query->corpus GT pairs, eval same 2000 queries),
raw-WJ target (matches gt_lookup_10k = RAW-vector WJ). One 4096-d model, loss on nested prefixes
{256,512,1024,2048,4096}. Eval: WJ HNSW + exact-WJ rerank + QPS, R@10/50/500 per dim.
Launch: CUDA_VISIBLE_DEVICES=7 python run_matdistill_10k.py"""
import sys, time, csv, datetime, random, pickle
...
def main():
    ...
    print(f"10K corpus={qs} q={len(qq)} dim={IN}",flush=True)
    ...
    for ep in range(1,EPOCHS+1):
        ...
        if tl/st<best: best=tl/st; torch.save(m.state_dict(),'/tmp/best_matdistill_10k.pt')

    # ---- eval: each prefix -> WJ HNSW + rerank + QPS, R@10/50/500 vs gt_lookup_10k ----
    m.load_state_dict(torch.load('/tmp/best_matdistill_10k.pt',map_location=DEV,weights_only=True)); m.eval()
    with torch.no_grad():
    ...
    note=f"MatDistill 10K prefixes={PREFIXES}; RAW-WJ target; efSearch={EF}"
    with open(CSV,"a",newline="") as f:
        w=csv.writer(f)
        for k,stage,ck,r10,r50,r100,r500,qps in rows:
            w.writerow([today,"10k",f"MatDistill10K-d{k}",k,stage,ck,round(r10,4),round(r50,4),round(r100,4),round(r500,4),qps,"run_matdistill_10k.py",note])
