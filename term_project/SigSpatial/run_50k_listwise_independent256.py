#!/usr/bin/env python3
"""Independent (non-Matryoshka) 256-d listwise training at 50K -- isolates whether
our actual deployed operating point (256-d) is left leaving accuracy on the table
because it's a truncated slice of a jointly-trained 4096-d model, rather than a
model natively built for 256-d. Unlike the 4096-d independent script (same
architecture, single loss term), this one is genuinely architecturally smaller:
the encoder's output layer produces 256-d directly, not 4096-d truncated to 256.
Same data/optimizer/epoch budget/listwise loss/tau as run_50k_listwise.py otherwise.

Compare to the existing Matryoshka-256 slice (R@50=0.921). If this reaches
meaningfully higher recall, our current 256-d deployment recommendation is leaving
real accuracy on the table by using a truncated Matryoshka slice instead of a
natively-sized model.

Launch: CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --standalone --nproc_per_node=4 run_50k_listwise_independent256.py"""
...
CKPT = '/raid/ruban/hpmlproj/term_project/SigSpatial/best_50k_listwise_indep256.pt'
...
class Net(nn.Module):
    """Natively 256-d output -- NOT a 4096-d encoder truncated to 256. Same overall
    shape/depth as the Matryoshka encoder otherwise (D->4096->DIM)."""
    def __init__(s, d):
        super().__init__(); s.encoder = nn.Sequential(nn.Linear(d, 4096, bias=False), nn.BatchNorm1d(4096), nn.ReLU(),
            nn.Linear(4096, DIM, bias=False), nn.BatchNorm1d(DIM))
...
def evaluate(embed_fn, gt, qs, Cr, Qr, Cr_sum, qtn, DEV, tag):
    ...
    note = ("Independent (non-Matryoshka) single-width listwise training, natively 256-d output "
            "(not a 4096-d truncation): same data/10ep/listwise loss/tau as run_50k_listwise.py. "
            "Isolates whether our actual 256-d deployment recommendation leaves accuracy on the "
            "table vs. the Matryoshka-256 slice (R@50=0.921).")
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for m, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "pk50k", f"50k-listwise-indep256", m, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_50k_listwise_independent256.py", note])
...
def main():
    ...
    if main: print(f"DDP world={world} method=listwise-independent-256 corpus={QS} queries={qt.shape[0]-QS} dim={IN}", flush=True)
    ...
    evaluate(enc.embed, gt, QS, Cr, Qr, Cr_sum, QTN, DEV, 'listwise-indep256')
