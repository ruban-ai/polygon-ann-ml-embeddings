...
tried (\S\ref{sec:distill}), so it is our primary method throughout. Against
ShapeToVec's own published results~\cite{shape2vec} on the identical Parks dataset
(233,773 polygons, same 80/20 corpus/query split), our 256-dimension embedding, $12$--
$47\times$ smaller than their tested $3$k--$12$k-dimension resolutions, reaches
Recall@50 $=92.1\%$ without reranking (vs.\ their $96$--$97\%$) at $4$--$14\times$ the
throughput, and $99.7\%$ after a cheap exact-WJ rerank pass, exceeding their best
published recall while still running several-fold faster than their most accurate
configuration. The same encoder, unchanged, generalises to a second, structurally
different polygon class (water bodies), where its margin over random projection is if
anything larger.
\end{abstract}
