...
tried (\S\ref{sec:distill}), so it is our primary method throughout. This is, to
our knowledge, the first learned WJ embedding to surpass random projection on both
the near- and wide-net recall--throughput frontiers: on a 50K benchmark it reaches
Recall@500 $=0.969$ ($0.990$ after reranking) at just 256 dimensions ($72\times$
compression), beating random projection at every dimension while running
competitively fast. The same encoder, unchanged, generalises to a second,
structurally different polygon class (water bodies), where its margin over random
projection is if anything larger.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  ...
  \item A \textbf{shape-similarity distillation loss}: WJ-native distillation makes
    the training objective the retrieval metric itself. We compare absolute value
    regression (MSE) against a rank-aware \emph{listwise} objective and find the
    latter wins decisively at every scale tested and against five further
    2024--2025 rank-aware alternatives (\S\ref{sec:distill}); either variant is,
    to our knowledge, the first learned WJ embedding to beat an untrained random
    projection (\S\ref{sec:exp}).
...
\begin{equation}
  ...
  \mathcal{L}_{\mathrm{list}}=-\frac{1}{N}\sum_i\sum_{j\neq i}
    p_{ij}\log\hat p_{ij},\quad \hat p_{ij}=\frac{\exp(\hat T_{ij}/\tau)}
...
WJ-distillation (MSE)$^\ddagger$        & 512  & 0.844 & 0.930 & 0.994 & 14{,}552 \\
\textbf{WJ-distillation (listwise)}$^\ddagger$ & \textbf{512} & \textbf{0.945} & \textbf{0.973} & \textbf{0.996} & \textbf{9{,}690} \\
\bottomrule
\end{tabular}
\end{table}
...
WJ-distill.\ (MSE)$^\ddagger$       & --- & 0.692 & 0.794 & 0.924 & 6{,}772 \\
\textbf{WJ-distill.\ (listwise)}$^\ddagger$ & --- & \textbf{0.871} & \textbf{0.921} & \textbf{0.969} & \textbf{4{,}921} \\
\midrule
...
WJ-distill.\ (MSE)$^\ddagger$       & 2000 & 0.995 & 0.997 & 0.976 & 1{,}253 \\
\textbf{WJ-distill.\ (listwise)}$^\ddagger$ & 2000 & \textbf{0.995} & \textbf{0.997} & \textbf{0.990} & \textbf{1{,}083} \\
\bottomrule
\end{tabular}
\end{table}
...
WJ-distill.\ (MSE)$^\ddagger$       & --- & 0.722 & 0.815 & 0.930 & 4{,}632 \\
\textbf{WJ-distill.\ (listwise)}$^\ddagger$ & --- & \textbf{0.858} & \textbf{0.912} & \textbf{0.965} & \textbf{6{,}293} \\
\midrule
\multicolumn{6}{@{}l@{}}{\textit{Stage 2 --- exact-WJ rerank}}\\[1pt]
Rand.\ Proj.~\cite{achlioptas03}    & 2000 & 0.994 & 0.994 & 0.958 & 362 \\
WJ-distill.\ (MSE)$^\ddagger$       & 2000 & 0.995 & 0.997 & 0.987 & 1{,}280 \\
\textbf{WJ-distill.\ (listwise)}$^\ddagger$ & 2000 & \textbf{0.995} & \textbf{0.997} & \textbf{0.994} & \textbf{1{,}300} \\
\bottomrule
\end{tabular}
\end{table}
...
\addlinespace[2pt]
\textbf{Distill (listwise)} 256$^\ddagger$ & \textbf{0.871} & \textbf{0.921} & \textbf{0.969} & 4{,}921 \\
\textbf{Distill (listwise)} 512$^\ddagger$ & 0.877 & 0.925 & 0.971 & 2{,}682 \\
\textbf{Distill (listwise)} 1024$^\ddagger$ & 0.880 & 0.928 & 0.972 & 1{,}958 \\
\textbf{Distill (listwise)} 2048$^\ddagger$ & 0.881 & 0.929 & 0.973 & 1{,}631 \\
\textbf{Distill (listwise)} 4096$^\ddagger$ & 0.881 & 0.929 & 0.973 & 1{,}493 \\
\bottomrule
\end{tabular}
\end{table}
