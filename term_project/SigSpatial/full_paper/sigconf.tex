...
aligning the training objective with the retrieval metric directly, so the deployed
embedding's WJ reproduces the true WJ, and yielding a single Matryoshka encoder
deployable at any dimension. Within this framework we compare two distillation
objectives, absolute value regression (MSE) and a rank-aware \emph{listwise}
objective that matches each anchor's full similarity distribution rather than
individual values; the listwise variant wins decisively at every scale we test
(10K/50K/187K polygons) and against every 2024--2025 rank-aware alternative we
tried (\S\ref{sec:distill}), so it is our primary method throughout. Compression here
is not the usual accuracy-for-size trade: on a 50K benchmark, reproducing ShapeToVec's
own method exactly, uncompressed ($D{=}18{,}382$), directly under nmslib's Weighted
Jaccard HNSW space, reaches only Recall@50 $=0.699$ (Recall@500 $=0.817$); our
$72\times$-smaller, 256-dimension embedding reaches Recall@50 $=0.921$ (Recall@500
$=0.969$, $0.990$ after reranking) on the identical corpus, queries, and ground truth,
at several-fold higher throughput. Approximate nearest-neighbour search degrades in
ShapeToVec's native high-dimensional space; a well-shaped compact embedding is not
merely cheaper to search, it is an easier space to search. The same encoder,
unchanged, generalises to a second, structurally different polygon class (water
bodies), where its margin over random projection is if anything larger.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  \item \textbf{Compression that improves accuracy, not just cost.} Reproducing
    ShapeToVec's own method, uncompressed, directly on our benchmark shows that
    approximate search over its native high-dimensional space is itself a bottleneck:
    our $72\times$-smaller learned embedding beats the uncompressed baseline's recall
    outright (\S\ref{sec:exp}), at several-fold higher throughput. Compression here is
    not an accuracy-for-size trade; a well-shaped compact space is easier for
    approximate nearest-neighbour search to navigate, not merely cheaper to store.
  \item A \textbf{shape-similarity distillation loss} that makes this possible: WJ-native
    ...
    2024--2025 rank-aware alternatives (\S\ref{sec:distill}).
...
vs.\ $0.794$; 187K/256-d R@50 $0.906$ vs.\ $0.797$), unlike the pairwise/margin
...
2024--2025 rank-aware and Matryoshka-training refinements sourced from recent
literature (sequential/curriculum Matryoshka training~\cite{smec25}, an RKD-style
angle-wise geometric term~\cite{park19rkd}, cross-batch hard-negative
memory~\cite{smec25}, learnable dimension selection~\cite{smec25}, and
cross-dimension self-distillation~\cite{mipic25}); none improved on plain listwise,
and three actively hurt retrieval quality (full results and mechanistic analysis in
...
50K/256-d R@50 $0.921$ vs.\ $0.794$; 187K/256-d R@50 $0.906$ vs.\ $0.797$ (full
...
187K (256-d) & 0.797 & \textbf{0.906} & $+10.9$\,pp \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Compression versus the uncompressed baseline (Table~\ref{tab:uncompressed}).}
Every comparison above is against \emph{other compressed} representations (random
projection, contrastive baselines, MSE distillation); none test whether compression
itself costs anything relative to ShapeToVec's own, uncompressed method. We reproduce
that method exactly, the full $D{=}18{,}382$-dim probability-simplex vector
(\S\ref{sec:embed}), indexed directly under nmslib's WeightedJaccard HNSW space with no
learned compression, on the identical 50K corpus, queries, and ground truth used
everywhere else in this paper. It reaches only R@50 $=0.699$ and R@500 $=0.817$ at
$1{,}102$ QPS; our $72\times$-smaller 256-d embedding reaches R@50 $=0.921$ and R@500
$=0.969$ at $4{,}921$ QPS -- a $+22.2$pp and $+15.2$pp recall margin, at $4.5\times$ the
throughput. This is not the usual accuracy-for-size trade: compression \emph{improves}
recall here. The WJ metric itself is not the bottleneck, since the quadtree encoding
already approximates it near-exactly (\S\ref{sec:exp}); a brute-force sketch estimator,
ICWS (Table~\ref{tab:results}), confirms this directly, reaching R@500 $=0.924$ at 50K,
much closer to the exact ceiling than uncompressed HNSW's $0.817$, precisely because it
ranks exhaustively rather than searching a graph. The bottleneck is specifically
\emph{approximate search over a high-dimensional space}: HNSW navigates greedily via
locally meaningful distance differences, and that signal weakens as dimension grows
(distance concentration). A learned, lower-dimensional embedding is not merely cheaper
to search, its geometry is easier for approximate search to get right, which is why our
compressed pipeline beats \emph{both} the uncompressed HNSW baseline (on recall and
speed) and the brute-force sketch estimator (at two orders of magnitude higher
throughput, $4{,}921$ vs.\ $20$ QPS).

\begin{table}[t]\centering
\caption{Compression versus the uncompressed baseline (50K benchmark, Stage-1, no
rerank). Uncompressed reproduces ShapeToVec's own method exactly: full $D{=}18{,}382$-dim
simplex vectors, no learned compression, indexed directly under nmslib's WeightedJaccard
HNSW space, on the identical corpus/queries/GT as every other 50K result in this paper.
$\ddagger$~=~ours. \textbf{Bold}~=~best.}
\label{tab:uncompressed}\setlength{\tabcolsep}{4pt}\small
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method (dim) & R@10 & R@50 & R@500 & QPS \\
\midrule
Uncompressed, 18{,}382-d      & 0.649 & 0.699 & 0.817 & 1{,}102 \\
\textbf{Listwise distill, 256-d}$^\ddagger$ & \textbf{0.871} & \textbf{0.921} & \textbf{0.969} & \textbf{4{,}921} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]\centering
\caption{The full 187k Parks corpus: WJ-distillation (listwise) on \emph{all}
46{,}754 held-out queries (corpus-trained), 256-d prefix. MSE row shown for
comparison (Table~\ref{tab:lossablation}).} %$\ddagger$~=~ours.}
...
compression, is the natural operating point. Baselines, by contrast, stay below
...
and compression here is not a trade against accuracy: on our 50K benchmark the compact
embedding beats ShapeToVec's own uncompressed method, reproduced exactly on the same
corpus, queries, and ground truth, by $+22.2$pp R@50, at several-fold higher throughput,
because approximate search itself degrades in ShapeToVec's native high-dimensional
space. Contrastive objectives work at moderate scale but collapse to random-projection
level at 50K, since they optimise separation rather than the WJ ranking recall needs.
WJ-native distillation removes this misalignment, and a rank-aware \emph{listwise}
formulation of it beats absolute-value regression decisively at every scale we test
(10K/50K/187K) and against five further 2024--2025 rank-aware alternatives: a single
Matryoshka encoder beats random projection, ShapeToVec's own uncompressed method, and a
brute-force sketch estimator at both near- and wide-net recall, confirmed up to 187k
