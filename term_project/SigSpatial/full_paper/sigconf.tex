...
encoding each polygon as a high-dimensional probability-simplex vector indexed directly
under WJ with HNSW, but that high dimensionality costs heavily in memory and query
throughput. We introduce \emph{WJ-native distillation}: a simplex-constrained encoder
that maps each polygon's raw vector onto a compact probability simplex. It is trained
with a rank-aware \emph{listwise} loss: each anchor's row of true WJ similarities is
turned into a target distribution, and the compressed embedding's own WJ row is trained
to match it, supervising the full ranking structure rather than individual values. This
beats absolute-value regression and five further rank-aware and Matryoshka-training
alternatives at every scale we test. A single trained encoder serves every deployed
width via Matryoshka-style truncation, so one training run covers every operating point
from 256 to 4096 dimensions. Against ShapeToVec's own
