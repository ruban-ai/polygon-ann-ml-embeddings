...
complaint before.}\rs{Done -- changed to ``floating-point vector'' here;
\emph{simplex} is now used only for our own embedding, below.} indexed directly
...
ability and reduced accuracy.}\rs{Done -- see the next sentence: 32$\times$ smaller,
recall $96\%\to76\%$, cited.}
...
what you do in your code}\rs{Done -- reworded to ``true, exact-WJ-ranked
neighbours'' rather than the row/softmax mechanics.}
...
knowing what these mean}\rs{Done -- removed the specific method names here; they're
properly defined in Contribution 2 and the ablation tables.}
A single trained encoder serves every
deployed width via Matryoshka-style truncation; one training run
\ba{replace so as it does not sound formal}\rs{Done -- replaced ``so'' with
``therefore''.}
therefore covers
...
-- adopted \emph{designed} vs.\ \emph{learned} signatures as the framing above
(``Both rely on designed signatures... We instead introduce learned signatures via
WJ-native distillation''), keeping ``distillation'' as the name for the specific
mechanism. Let me know if that's not the split you meant.}
\end{abstract}
...
operation, for deduplication, conflation, and map integration; concretely, tracking
how a lake shrinks or an ice shelf fragments over time requires matching each new
observation to its closest-shaped counterpart in a large historical
archive~\cite{shape2vec}. We study
...
by cell over a shared spatial grid (formalised as Eq.~\ref{eq:wj}, \S\ref{sec:related})
-- is the standard choice. The core difficulty is
...
none produce WJ-compatible embeddings. We deliberately keep the index itself
unchanged -- ShapeToVec's HNSW pipeline, not a hashing-based ANN scheme -- and
compress only the vector representation searched within it. Our contributions are as follows:
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  \item \textbf{The first learned, shape-aware compression for WJ polygon retrieval.}
    ...
    several-fold faster than their most accurate configuration (\S\ref{sec:exp}).
  ...
  \item \textbf{Supporting design}: a WJ-native simplex encoder (ReLU\,+\,$\ell_1$
    ...
    dominates the recall--throughput frontier. The width range itself is a
    training-time configuration, not a fixed architectural choice: serving a different
    range, e.g.\ narrower than 256-d, needs no redesign, only retraining with that
    range.
\end{enumerate}
...
decisively at every scale we test (10K/50K/233K) and against five further rank-aware
and Matryoshka-training alternatives: a single Matryoshka encoder beats random projection and a
