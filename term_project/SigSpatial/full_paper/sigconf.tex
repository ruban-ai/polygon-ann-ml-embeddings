...
encoding each polygon as a high-dimensional floating-point vector indexed directly
under WJ with HNSW, but that high dimensionality costs heavily in memory and query
throughput. ShapeToVec+~\cite{shapetovecplus26} compresses these into compact
...
each anchor's true, exact-WJ-ranked neighbours guide training, supervising the full
ranking structure rather than individual values. This beats several alternative
training objectives we test, at every scale. A single trained encoder serves every
deployed width via Matryoshka-style truncation; one training run therefore covers
