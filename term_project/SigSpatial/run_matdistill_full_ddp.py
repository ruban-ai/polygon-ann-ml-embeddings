#!/usr/bin/env python3
"""187K full-corpus WJ-native MATRYOSHKA distillation — DDP (true 8-GPU).
Identical protocol to matryoshka_triplet/infonce_train80 (train80/eval20 held-out via
/tmp/query_split_80_20.pkl, eval vs full GT = RAW-vector WJ) so it slots directly into
Table 2 / Table 4. Target = per-batch raw-WJ (187K^2 can't be precomputed); loss on nested
prefixes {256,512,1024,2048,4096}. Each rank runs its own batch+loss; grads synced.
Launch: CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 torchrun --standalone --nproc_per_node=7 run_matdistill_full_ddp.py"""
...
from sota_experiment_common import (load_dataset_normalized, nmslib_neighbors,
    preload_rerank_corpus, release_rerank_corpus, rerank_wj_gpu, eval_recall_by_qids)
...
def main():
    ...
    if main: print(f"DDP world={world} | corpus={qs} train={len(TR)} eval={len(EV)} dim={IN}",flush=True)
    ...
    for ep in range(1,EPOCHS+1):
        ...
        if main:
            ...
            if tl/st<best: best=tl/st; torch.save(model.module.state_dict(),'/tmp/best_matdistill_full.pt')
    dist.barrier()
    if not main: dist.destroy_process_group(); return

    # ---- eval on rank 0: each prefix -> WJ HNSW + rerank + QPS, HELD-OUT eval20 vs full GT ----
    enc=Net(IN).to(DEV); enc.load_state_dict(torch.load('/tmp/best_matdistill_full.pt',map_location=DEV,weights_only=True)); enc.eval()
    ...
    note=f"MatDistill-DDP full eval20 n={len(ei)} prefixes={PREFIXES}; RAW-WJ target; efSearch={EF}"
    with open(CSV,"a",newline="") as f:
        w=csv.writer(f)
        for m,stage,ck,r10,r50,r100,r500,qps in rows:
            w.writerow([today,"full",f"MatDistillDDP-d{m}",m,stage,ck,round(r10,4),round(r50,4),round(r100,4),round(r500,4),qps,"run_matdistill_full_ddp.py",note])
