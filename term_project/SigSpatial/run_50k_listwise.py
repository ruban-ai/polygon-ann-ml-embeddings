#!/usr/bin/env python3
"""B4 ablation at 50k scale: does the listwise (rank-aware) loss that beat MSE at
10k also beat it at 50k, or does it collapse like the other ranking-style
objectives (triplet, InfoNCE) already shown to fail at this scale (Table 2)?
Identical protocol to run_50k_ddp.py --method distill (same architecture, corpus
self-kNN pair mining, 40k corpus / 10k held-out queries, epochs=10) -- only the
loss function differs (listwise cross-entropy instead of MSE).
Launch: CUDA_VISIBLE_DEVICES=0..7 torchrun --standalone --nproc_per_node=8 run_50k_listwise.py"""
...
def evaluate(embed_fn, gt, qs, Cr, Qr, Cr_sum, qtn, DEV, tag):
    ...
    note = f"pk-real50k0.002 listwise(B4 ablation) corpus-trained, eval held-out queries; raw-WJ; tau={TAU}; efSearch={EF}"
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for m, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "pk50k", f"50k-listwise-d{m}", m, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_50k_listwise.py", note])
