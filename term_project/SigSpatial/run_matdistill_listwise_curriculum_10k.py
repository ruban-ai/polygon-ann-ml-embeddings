#!/usr/bin/env python3
"""SMRL-style ablation on top of listwise (10K): SMEC (EMNLP 2025) shows joint
multi-width Matryoshka training creates gradient-variance noise on shared
parameters, since every width's loss pushes the same shared weights with a
different magnitude every step, and that sequential (not joint) width training
was their single largest ablation gain. Our encoder can't do SMEC's literal
freeze-and-stack (one shared dense output layer produces all widths at once,
not separate stacked adapters per width) -- this approximates the same spirit
with a CURRICULUM: widen which prefixes get supervised in stages, instead of
supervising all five {256,512,1024,2048,4096} jointly from epoch 1 (as
run_matdistill_listwise_10k.py does).

Same architecture / data / total epoch budget (40) / optimizer / listwise loss
as run_matdistill_listwise_10k.py -- only the *training schedule* differs, for
a controlled comparison isolating this one change.

Launch: CUDA_VISIBLE_DEVICES=2 python run_matdistill_listwise_curriculum_10k.py"""
...
TAU = 0.1
EPOCHS_PER_STAGE = EPOCHS // len(PREFIXES)  # 8 epochs/stage, 5 stages, same 40-epoch total budget
EF = 200; RB = 64; CAND_KS = [500, 1000]  # NOTE: 100 dropped -- causes KeyError in eval_recall (max_k caps below 500)
SEED = 42
CSV = "/raid/ruban/hpmlproj/term_project/SigSpatial/NEW_RESULTS.csv"
CKPT = '/raid/ruban/hpmlproj/term_project/SigSpatial/best_matdistill_listwise_curriculum_10k.pt'
...
def listwise_curriculum(z, tw, active_prefixes):
    """Same listwise loss as run_matdistill_listwise_10k.py, but averaged only
    over `active_prefixes` (the widened subset for the current stage), not
    always all of PREFIXES."""
    N = z.shape[0]
...
def main():
    ...
    print(f"10K corpus={qs} q={len(qq)} dim={IN} loss=listwise-curriculum tau={TAU} "
          f"stages={len(PREFIXES)} epochs/stage={EPOCHS_PER_STAGE}", flush=True)
    ...
    for ep in range(1, EPOCHS + 1):
        stage = min((ep - 1) // EPOCHS_PER_STAGE, len(PREFIXES) - 1)  # 0-indexed stage
        active_prefixes = PREFIXES[:stage + 1]  # cumulative widening: {256} -> {256,512} -> ... -> all 5
        m.train(); tl = st = 0
    ...
    for k in PREFIXES:
        ...
        print(f"[curriculum d={k} base] R@10={base[10]:.4f} R@50={base[50]:.4f} R@500={base[500]:.4f} HNSW_QPS={info['qps']:.0f}", flush=True)
        ...
        for ck in CAND_KS:
            ...
            print(f"[curriculum d={k} rerank K={ck}] R@10={mm[10]:.4f} R@50={mm[50]:.4f} R@500={mm[500]:.4f} e2eQPS={e2e:.0f}", flush=True)
            rows.append([k, "rerank", ck, mm[10], mm[50], mm[100], mm[500], round(e2e)])
        release_rerank_corpus()
    note = (f"SMRL-curriculum ablation vs MatDistillListwise10K: same arch/data/40ep budget, "
            f"widths widened in {len(PREFIXES)} stages of {EPOCHS_PER_STAGE}ep instead of joint training; "
            f"RAW-WJ target; tau={TAU}; efSearch={EF}")
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for k, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "10k", f"ListwiseCurriculum10K-d{k}", k, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_matdistill_listwise_curriculum_10k.py", note])
