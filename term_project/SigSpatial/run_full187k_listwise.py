#!/usr/bin/env python3
"""B4 ablation at the full 187k Parks corpus: the listwise loss beat MSE
distillation at both 10k and 50k (not a triplet/InfoNCE-style collapse) --
this is the paper's actual scaling-check headline benchmark (all 46,754 queries
held out, corpus-trained). Identical protocol to run_matdistill_fulleval_ddp.py,
only the loss function differs (listwise cross-entropy instead of MSE).
Launch: CUDA_VISIBLE_DEVICES=0..7 torchrun --standalone --nproc_per_node=8 run_full187k_listwise.py"""
...
B = 512; EPOCHS = 5; LR = 1e-3; WD = 1e-4; MAX_POS = 30; TAU = 0.1
...
CKPT = '/raid/ruban/hpmlproj/term_project/SigSpatial/best_full187k_listwise.pt'
...
def main():
    ...
    if main: print(f"DDP world={world} loss=listwise tau={TAU} corpus={qs} queries={len(qq)} dim={IN}", flush=True)

    knn = np.load(KNN_CACHE)
    if main: print(f"corpus kNN {knn.shape} (cached)", flush=True)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        sch.step()
        if main:
            print(f"ep{ep:02d} listwise_ce={tl/st:.6f} {(time.time()-t0)/60:.1f}min", flush=True)
            if tl / st < best: best = tl / st; torch.save(model.module.state_dict(), CKPT)
    dist.barrier()
    if not main: dist.destroy_process_group(); return

    # ---- eval on rank 0: ALL 46,754 queries vs 187K corpus ----
    ...
    for m in PREFIXES:
        ...
        print(f"[listwise d={m} base ALLq] R@10={base[10]:.4f} R@50={base[50]:.4f} R@500={base[500]:.4f} HNSW_QPS={info['qps']:.0f}", flush=True)
        ...
        for ck in CAND_KS:
            ...
            t1 = time.time(); rr = rerank_wj_gpu(qq, cand, cq, cs, top_k=ck, batch_size=RB)
            e2e = len(qq) / max(ci["query_s"] + (time.time() - t1), 1e-9)
            mm = eval_recall(gt, rr, qs, ck)
            print(f"[listwise d={m} rerank K={ck} ALLq] R@10={mm[10]:.4f} R@50={mm[50]:.4f} R@500={mm[500]:.4f} e2eQPS={e2e:.0f}", flush=True)
            rows.append([m, "rerank", ck, mm[10], mm[50], mm[100], mm[500], round(e2e)])
        release_rerank_corpus()
    note = f"Listwise(B4 ablation) full-eval ALL {len(qq)} queries, corpus-trained, prefixes={PREFIXES}; raw-WJ GT; tau={TAU}; efSearch={EF}"
    with open(CSV, "a", newline="") as f:
        w = csv.writer(f)
        for m, stage, ck, r10, r50, r100, r500, qps in rows:
            w.writerow([today, "full_allq", f"ListwiseFull-d{m}", m, stage, ck, round(r10, 4), round(r50, 4), round(r100, 4), round(r500, 4), qps, "run_full187k_listwise.py", note])
