...
- **Stability note (two rounds):** first launch diverged to NaN within epoch 1. Round 1 fix (separate grad-norm clip for `imp`, 10x lower LR, hard-clamp ±15) targeted gradient *magnitude* but the identical NaN recurred deterministically — meaning the real bug was a NaN *gradient* (0/0-style), which clamping a finite value can't fix. Root cause: `norm()`'s `clamp(min=1e-10)` denominator floor is fine under the *trained* first-m prefix the other scripts use, but ADS's near-random early selection (imp starts at 0) makes it plausible for a row's chosen top-m columns to be ALL exactly zero post-ReLU — dividing by a ~1e-10 floor there produces a ~1e10 gradient in the backward pass. Round 2 fix: dedicated `norm_masked()` with a 1e-4 floor for the ADS masking path specifically, plus a defensive `nan_to_num_` on all gradients before clipping as a backstop. Relaunched. Results pending.
