...
### 3.4 ADS — learnable dimension selection (candidate #4) — DONE, verdict: rejected, largest regression of the four
- **Script:** `run_matdistill_listwise_ads_10k.py`. One shared learnable importance vector over EMB=4096 dims, straight-through Gumbel top-m mask per Matryoshka width replaces static first-m prefix truncation.
- **Stability note (three rounds) — root cause found and fixed:** first two launches diverged to NaN within epoch 1, identically (round 1's gradient-magnitude fixes didn't help; round 2's denominator-floor theory was also wrong — `imp` stayed at exactly 0.000 rather than NaN, meaning the *forward pass itself* was NaN, not a backward-pass gradient explosion). Isolated the actual bug with a standalone repro against real batch data: `gumbel_like()` had a Python operator-precedence bug — `-torch.log(Y).clamp(min=eps)` parses as `-(torch.log(Y).clamp(min=eps))`, not `(-torch.log(Y)).clamp(min=eps)`. Since `torch.log(U)` for `U∈[1e-10,1)` is always negative, clamping it to `min=1e-10` forced every element to the same constant, and the outer `log()` of that (still negative after the outer negation) hit `log(negative)=NaN` uniformly, every single call — deterministic, hence identical failure both times. Fixed with explicit parens; verified standalone against 100K samples (mean=0.579, std=1.289, matching the theoretical Gumbel(0,1) mean=0.577/std=1.283 exactly). Training then ran cleanly for all 40 epochs.
- **Eval bug (caught before trusting the numbers):** the training script's own eval built the width-k embedding as `norm(Z * mask)` — a zero-padded 4096-dim vector with only k nonzero entries, not a genuinely compacted k-dim vector. This gave misleading QPS (e.g. "d=256" measured ~976 QPS, barely above the true 4096-d baseline's 1339, since nmslib was searching full 4096-dim vectors at every nominal width regardless of k). Recall itself was unaffected (zero-padding doesn't change the WJ ratio math) but the QPS/deployment-cost comparison was invalid. Fixed with a separate eval-only script (`eval_listwise_ads_10k.py`, reusing the trained checkpoint, no retraining needed) that actually gathers `Z[:, top-k indices]` into a real `(N,k)` array before indexing.

**Result (base, no rerank), all five widths — a real regression, the largest of any candidate tested:**

| Dim | Baseline R@10 | ADS R@10 | Δ | Baseline R@50 | ADS R@50 | Δ | Baseline QPS | ADS QPS |
|---|---|---|---|---|---|---|---|---|
| 256 | 0.9411 | 0.7866 | **−15.5pp** | 0.9715 | 0.8675 | **−10.4pp** | 14145 | 20209 |
| 512 | 0.9454 | 0.8383 | **−10.7pp** | 0.9734 | 0.9048 | **−6.9pp** | 9690 | 15522 |
| 1024 | 0.9477 | 0.8648 | **−8.3pp** | 0.9747 | 0.9244 | **−5.0pp** | 4562 | 5828 |
| 2048 | 0.9478 | 0.8850 | **−6.3pp** | 0.9751 | 0.9389 | **−3.6pp** | 2331 | 2367 |
| 4096 | 0.9481 | 0.9054 | **−4.3pp** | 0.9752 | 0.9536 | **−2.2pp** | 1339 | 1218 |

Rerank recovers R@10/R@50 fully (matches baseline at every width), but R@500 still trails at shallow rerank depth (e.g. d=256, K=500: 0.9811 vs baseline 0.9954), closing mostly at K=1000 — same pattern seen in RKD.

**Interesting side note:** ADS's compacted vectors are genuinely faster to search at the smaller widths (256-d: 20209 vs 14145 QPS, +43%) — the *learned* dimension selection does produce a real speed benefit at query time, just at a steep recall cost. Even at 4096-d (nominally "select everything," so the mask should be a no-op), there's still a real −4.3pp R@10 gap, most likely because the shared encoder's weights were shaped by the noisy Gumbel top-k training signal on the *other* widths every step, not because the 4096-d selection itself does anything.

**Verdict: rejected — the most severe regression among all four "no stones unturned" candidates.** The straight-through Gumbel top-k mechanism, even after fixing two real implementation bugs (the NaN and the eval padding), produces embeddings that are meaningfully worse for ANN retrieval than simply using the trained first-k prefix. Plausible explanation: the noisy per-step re-selection (different columns "selected" each step depending on Gumbel noise, especially early in training) gives the encoder a much less stable, less consistent training signal per dimension than the static first-m convention the other scripts rely on — dimensions never get to specialize consistently the way a fixed Matryoshka prefix ordering does.
