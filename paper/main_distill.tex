...
% --- tighten whitespace around floats/captions (reclaim space, no content cut) ---
\setlength{\textfloatsep}{7pt plus 2pt minus 2pt}   % float <-> body text
\setlength{\floatsep}{6pt plus 2pt minus 2pt}       % float <-> float
\setlength{\intextsep}{6pt plus 2pt minus 2pt}      % in-text floats
...
WJ-compatible compression employing a simplex-constrained encoder, with a WJ-native triplet loss with false-negative filtering and decoder reconstruction, a 36$\times$ smaller 512-dimensional vectors and, on a 10k
subset of the Parks dataset, achieves Recall@50 $=0.858$ at 10{,}903 QPS (78$\times$
faster than brute-force Improved Consistent Weighted Sampling (ICWS)) and Recall@10
$>0.996$ following exact-WJ reranking. However, on a larger 
benchmark, such ranking objectives no longer beat an untrained random projection since
they optimize separation rather than the WJ ranking, where recall depends on, and the loss
