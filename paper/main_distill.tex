...
(triplet, reconstruction, InfoNCE) is strong on a 10k subset but, on a larger
held-out 50K benchmark, is \emph{no better than an untrained random projection}:
these are separation proxies that do not preserve the WJ ranking recall depends on,
and the loss even damages the deployed output layer. We instead \emph{distill the
metric} --- regressing the embedding's WJ onto the true WJ aligns the objective with
the retrieval metric. A single Matryoshka~\cite{matryoshka22} encoder then surpasses
random projection at \emph{both} near- and wide-net recall: on the 50K benchmark
(40k corpus, 10k held-out queries), Stage-1 Recall@500 of 0.924 vs.\ 0.77 and 0.976
vs.\ 0.97 after reranking, at higher throughput and already at 256 dimensions. It
scales to the full 187k corpus (R@500 0.890 / 0.966 on all 46{,}754 held-out
queries) and reaches 10k Recall@50 0.93 (above brute-force ICWS) at 14.5k QPS.
\end{abstract}
