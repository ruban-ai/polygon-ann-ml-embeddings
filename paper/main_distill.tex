...
does \emph{not} guarantee the global WJ \emph{ranking} that recall depends on.
Empirically their recall collapses to that of an untrained random projection, and
the loss \emph{damages the very layer it is applied to}, so the deployed output is
not the network's best representation (\S\ref{sec:exp}). This motivates supervising
the metric itself.
