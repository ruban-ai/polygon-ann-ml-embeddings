...
ShapeToVec~\cite{shape2vec} encodes a polygon leveraging a quad-tree-based non-uniform grid, accounting
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  ...
  \item \textbf{Matryoshka deployment}: a single trained model serves every
    embedding width $\{256,\dots,4096\}$, giving the full recall--throughput frontier
    --- 256-d already dominates --- from one training.
\end{enumerate}
...
reaching $97\%$ R@50 (versus $67\%$ for its bit-vector variant). That accuracy,
...
\begin{tikzpicture}[font=\small,>={Latex[length=1.6mm,width=1.2mm]},
  ...
  dbox/.style={box,fill=orange!8,draw=orange!55}]
 ...
 \node[dbox,below=5mm of z] (dec) {decoder $g_\phi$\\{\scriptsize reconstruct $\mathbf{x}$}};
 ...
 \draw[->,dashed,orange!80](z)--node[right=0.4mm]{$\mathcal{L}_{\mathrm{rec}}$}(dec);
 \draw[->](z)--(h); \draw[->](h)--(r);
\end{tikzpicture}}
\caption{WJ-native pipeline. The encoder maps a quadtree vector onto the
probability simplex. Dashed boxes are two \emph{alternative} training routes
(training-only): WJ-distillation regresses the embedding's WJ onto the exact
$D$-dim WJ teacher (top); the contrastive autoencoder reconstructs $\mathbf{x}$
through a decoder (bottom). Retrieval (solid) is HNSW Stage-1 then exact-WJ rerank.}
\label{fig:pipeline}
\end{figure}
...
\subsection{Contrastive autoencoder training}
\label{sec:contrastive}
Our first route trains the encoder $f_\theta$ as a \emph{contrastive autoencoder},
pairing it with a decoder $g_\phi$ and supervising the embedding with a contrastive
loss plus a reconstruction term. With anchor--positive pairs and in-batch
...
\begin{equation}
  ...
  \label{eq:triplet}
\end{equation}
separates positives from negatives. A non-saturating InfoNCE~\cite{oord18},
\begin{equation}
  \mathcal{L}_{\mathrm{NCE}}=-\log\frac{\exp(S_{ii}/\tau)}
       {\sum_{j:(i,j)\notin\mathcal{G}}\exp(S_{ij}/\tau)},
  \label{eq:infonce}
\end{equation}
is an alternative. A reconstruction term
$\mathcal{L}_{\mathrm{rec}}{=}\|g_\phi(\mathbf{z})-\mathbf{x}\|_2^2$ regularises the
embedding; the full objective is $\mathcal{L}_{\mathrm{trip}}{+}\lambda\mathcal{L}_{\mathrm{rec}}$,
with the decoder dropped at inference. All variants are strong at 10k
(Table~\ref{tab:results}). Yet they optimise only \emph{separation} --- pushing
positives above negatives --- not the global WJ \emph{ranking} that recall depends on.
Empirically their recall collapses to an untrained random projection's. Worse, the
loss \emph{damages the very layer it is applied to} (\S\ref{sec:exp}). This motivates
supervising the metric itself.
...
sourced from SpatialHadoop~\cite{eldawy15}) --- at its \emph{highest-fidelity}
resolution ($D{=}18{,}382$ dimensions for the 50K benchmark, ${\sim}18.2$--$18.5$k
across subsets): the finest quadtree best preserves shape detail and gives
the strongest WJ recall, but is also the costliest to store and index --- making it
the most valuable compression target. The main benchmark is a 50K set partitioned into a 40k-polygon \emph{corpus} --- the
...
2\,TB RAM); QPS is measured on a single GPU.
...
\noindent\textbf{Why contrastive fails (Fig.~\ref{fig:layer}).}
The collapse is objective--metric misalignment, not a capacity limit. Within a
...
\noindent\textbf{The full Parks corpus.}
...
Stage-1 R@50/R@500 $=0.797/0.890$ at 2{,}136 QPS, rising to $0.995/0.966$ after
exact-WJ reranking (1{,}166 QPS).
...
\noindent\textbf{Trade-off between dimension and throughput (Fig.~\ref{fig:dim}, Table~\ref{tab:dim}).}
