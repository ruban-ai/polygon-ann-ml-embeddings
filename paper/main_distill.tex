...
\begin{abstract}
Similarity search over polygonal datasets is a critical spatial-data operation, with
Weighted Jaccard (WJ) capturing area-based shape overlap. The current state-of-the-art
ShapeToVec encodes a polygon leveraging a quad-tree-based non-uniform grid, accounting
for extreme area variations, employing a Hierarchical Navigable Small World (HNSW)
index for fast retrieval. However, their high dimensionality imposes a heavy memory
cost and query throughput penalty. We propose an efficient learned compact
WJ-compatible compression employing a simplex-constrained encoder. A contrastive
instantiation --- a WJ-native triplet loss with false-negative filtering and decoder
reconstruction --- gives 36$\times$ smaller 512-dimensional vectors and, on a 10k
subset of the Parks dataset, reaches Recall@50 $=0.858$ at 10{,}903 QPS (78$\times$
faster than brute-force Improved Consistent Weighted Sampling (ICWS)) and Recall@10
$>0.996$ following exact-WJ reranking. However, when scaling to a larger held-out
benchmark, such ranking objectives no longer beat an untrained random projection:
they optimize separation rather than the WJ ranking recall depends on, and the loss
even damages the deployed layer. We close this gap with \emph{WJ-native distillation}
--- regressing the embedding's WJ directly onto the true WJ --- which aligns the
training objective with the retrieval metric and yields a single Matryoshka encoder
deployable at any dimension. This is, to our knowledge, the first learned WJ embedding
to surpass random projection on both the near- and wide-net recall--throughput
frontiers at scale: on a 50K benchmark it reaches Recall@500 $=0.924$ ($0.976$ after
reranking) at just 256 dimensions (72$\times$ compression), surpassing random
projection at every dimension while running several-fold faster.
\end{abstract}
...
operation, underpinning data deduplication, conflation, and map integration. Such
polygons abound in open repositories: the park, water-body, and sports datasets we
study come from the SpatialHadoop GIS benchmark~\cite{eldawy15}, with further
large-scale sources such as Overture Maps and Google Open Buildings. Quantifying
...
Weighted Jaccard distance --- which ShapeToVec~\cite{shape2vec} adds to the NMSLIB
library~\cite{boytsov13} to make HNSW WJ-aware --- it reaches $97\%$ R@50
...
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt,topsep=1pt]
  \item A \textbf{WJ-native simplex embedding}: ReLU\,+\,$\ell_1$ outputs let WJ be
    computed directly under ShapeToVec's custom Weighted Jaccard distance in
    HNSW~\cite{shape2vec}, no post-processing.
  \item \textbf{A diagnosis} of why contrastive embeddings underperform for WJ
    retrieval: recall is governed by preserving the true WJ \emph{ranking}, not
    pairwise separation, so triplet/InfoNCE objectives are misaligned --- and the
    loss \emph{damages the layer it is applied to}, making the deployed output the
    worst layer in the network, even below an untrained random projection
    (\S\ref{sec:contrastive}).
...
\label{sec:embed}
The encoder is a two-layer MLP ($D\!\to\!4096\!\to\!4096$), its $4096$-d output
$\ell_1$-normalised and truncated to width $d$ at inference (Matryoshka,
\S\ref{sec:distill}). An MLP is deliberate: boundary Transformers~\cite{mai22}, vertex GNNs~\cite{polygongnn24}
...
the 50K benchmark (\S\ref{sec:exp}) their Stage-1 recall falls to that of an
untrained random projection. Worse, the loss \emph{damages the layer
...
\section{Experiments}
\label{sec:exp}
We use the Parks dataset~\cite{eldawy15} (233{,}773 polygons) encoded by
ShapeToVec~\cite{shape2vec}. The main benchmark is a 50K set (40k corpus / 10k queries); we also use a 10k subset (8k / 2k) for the moderate-scale
...
\noindent\textbf{The 50K benchmark (Table~\ref{tab:full50k}).}
...
\noindent\textbf{Scaling to the full corpus (Table~\ref{tab:full187}).}
The same encoder scales to the full 187k Parks corpus: trained only on corpus
neighbours and evaluated on \emph{all} 46{,}754 held-out queries, it reaches
...
%% ---------------- 5. Conclusion ----------------
\section{Conclusion}
We compress ShapeToVec WJ vectors up to $72\times$ with a WJ-native simplex
embedding. Contrastive objectives suffice at moderate scale but fall to
random-projection level on the held-out 50K benchmark --- they optimise separation,
not the WJ ranking recall needs, and damage the deployed layer. WJ-native
distillation removes the misalignment, and a single Matryoshka encoder beats random
projection at both near- and wide-net recall while running faster --- confirmed at
the full 187k-corpus scale, the first learned WJ embedding to do so. Future work
targets billion-scale corpora and other simplex-embeddable set metrics.
