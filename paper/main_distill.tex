...
polygons abound in open repositories: Natural Earth provides the park, water-body,
and sports-facility shapes we study, with further sources such as Overture Maps and
building-footprint collections (e.g.\ Google Open Buildings). Quantifying shape
overlap calls for an area-aware metric; Weighted Jaccard (WJ),
\begin{equation}
...
preserves overlap across scales; indexed with HNSW~\cite{hnsw20}
...
\section{Experiments}
\label{sec:exp}
We use OSM Parks polygons~\cite{osm,shape2vec}. The main benchmark is a 50K set
(40k corpus / 10k queries); we also use a 10k subset (8k / 2k) for the moderate-scale
