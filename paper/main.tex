...
Table~\ref{tab:ablation} isolates each design choice. The FN filter alone
slightly weakens the 8k signal (row 2): GT neighbours dominate the small batch,
leaving few true hard negatives --- an effect that vanishes at scale. WJ
regression \emph{without} filtering is catastrophic (R@10\,=\,0.510, row 3): the
regression anchors positives at high WJ while corrupted triplet gradients push
in-batch false negatives below the margin, a gradient conflict that filtering
resolves (row 4). Reconstruction is the decisive component (final row): unlike a
direct WJ target, it forces the encoder to preserve the full input distribution,
lifting R@50 to 0.858 (+5.0\,pp over triplet-only). The decoder is discarded at
inference.
