%%
%% ACM conference paper source generated from the official sigconf template.
%% Content integrated from the supplied manuscript.
%%
...
%% Rights management information. Replace DOI/ISBN with values from ACM rights confirmation before final submission.
...
%% Math support needed by the manuscript equations.
\usepackage{amsmath}
% \usepackage{amssymb}
\usepackage{booktabs}
\usepackage{graphicx}
...
\title{Weighted-Jaccard-Native Embedding Compression for Polygon Similarity Search under Extreme Area Variation}
...
%% -------------------------------------------------------
\begin{abstract}
Similarity search over polygonal datasets is a critical spatial-data operation, with Weighted Jaccard (WJ) capturing area-based shape overlap. The current state-of-the-art ShapeToVec encodes a polygon leveraging a quad-tree-based non-uniform grid, accounting for extreme area variations, employing a Hierarchical Navigable Small World (HNSW) index for fast retrieval. However, their dimensionality imposes a heavy memory cost and query throughput penalty. We propose an efficient learned compact 512-dimensional WJ-compatible compression employing a simplex-constrained encoder trained with a WJ-native triplet loss, false-negative filtering, and decoder reconstruction, giving 36× smaller vectors.
On a 10k subset of the Parks dataset, our method reaches Recall@50 = 0.858 at 10,903 QPS (78× faster than brute-force Improved Consistent Weighted Sampling (ICWS)) and Recall@10 > 0.996 following exact-WJ reranking. However, when scaling to the full Parks dataset, we observe that ranking no longer beats an untrained random projection. We close this gap by training the encoder as a recall-oriented candidate generator, a non-saturating contrastive objective over the broad neighbourhood, which restores recall and yields a more navigable index. This is, to our knowledge, the first learned WJ embedding to surpass random projection on the recall and query throughput frontiers at scale, with equal top-50 reranked recall at 1.9× higher throughput.
...
space~\cite{shape2vec} without post-processing.
...
$d=512$ embeddings using nmslib~\cite{hnsw20} and retrieves the top-$K$ candidates
per query.
...
Rand.\ Proj.~\cite{achlioptas03}      & --- & \textbf{0.628} & \textbf{0.663} & 0.722 &  1{,}275 \\
AE WJ-tri+recon$^\ddagger$            & --- & 0.566 & 0.656 & 0.656 &  1{,}188 \\
InfoNCE--WJ (ours)$^\ddagger$          & --- & 0.505 & 0.606 & \textbf{0.727} & \textbf{2{,}191} \\
\midrule
\multicolumn{6}{@{}l@{}}{\textit{Stage 2 — Exact WJ Rerank}} \\[1pt]
Rand.\ Proj.~\cite{achlioptas03}      & 1000 & 0.989 & 0.985 & \textbf{0.872} & 874 \\
AE WJ-tri+recon$^\ddagger$            & 1000 & \textbf{0.992} & 0.989 & 0.785 & 850 \\
\textbf{InfoNCE--WJ}$^\ddagger$ & 1000 & 0.992 & \textbf{0.992} & 0.854 & \textbf{1{,}108} \\
\addlinespace[3pt]
Rand.\ Proj.~\cite{achlioptas03}      & 2000 & 0.991 & 0.991 & \textbf{0.927} & 585 \\
InfoNCE--WJ $^\ddagger$         & 2000 & \textbf{0.992} & \textbf{0.992} & 0.863 & \textbf{730} \\
\bottomrule
\end{tabular}
\end{table}
...
HNSW $\text{efSearch}{=}200$). Rand.\ Proj.\ uses seed 42; learned models as labeled.
$\ddagger$~=~ours. \textbf{Bold}~=~best per column.}
\label{tab:dim-sweep}
...
Method & Dim & R@10 & R@50 & R@500 & QPS \\
\midrule
\multicolumn{6}{@{}l@{}}{\textit{Triplet + recon}} \\[1pt]
...
AE WJ-tri+recon$^\ddagger$ & 4096 & \textbf{0.603} & \textbf{0.691} & 0.699 & 1{,}145 \\
\midrule
\multicolumn{6}{@{}l@{}}{\textit{InfoNCE--WJ}} \\[1pt]
InfoNCE--WJ$^\ddagger$     &  512 & 0.505 & 0.606 & 0.727 & 2{,}191 \\
InfoNCE--WJ$^\ddagger$     & 2048 & 0.512 & 0.605 & 0.711 & 2{,}327 \\
InfoNCE--WJ$^\ddagger$     & 1024 & 0.559 & 0.649 & 0.746 & \textbf{2{,}602} \\
InfoNCE--WJ$^\ddagger$     & 2048 & 0.564 & 0.655 & \textbf{0.756} & 1{,}866 \\
...
as auxiliary loss ($\lambda$=1.0); AE rows use decoder reconstruction ($\lambda$=0.1). [TO BE UPDATED]}
\label{tab:ablation}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{lllrr}
