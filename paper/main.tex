...
parameters, of which $\sim$75M reside in the first layer). Three considerations
dictate this design, each driven by the structure of WJ and of the quadtree
input rather than by convention.
...
MLP trained \emph{natively} in WJ space avoids both --- it has no local
decomposition, and (below) its first layer is a single global projection.

\noindent\textbf{Global projection.}
Concretely, the first layer is a single dense projection
...
This is why $\sim$75M of the $\sim$80M parameters reside in this first layer.
