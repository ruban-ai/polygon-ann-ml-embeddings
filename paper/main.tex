...
ShapeToVec~\cite{shape2vec} encodes polygons as 18{,}219-dimensional probability
simplex vectors that preserve WJ, achieving 97\% R@50 on datasets up to 1.7M
polygons via direct HNSW indexing. However, indexing such large vectors becomes
increasingly expensive as corpora grow; compressing them while preserving WJ
structure enables faster search and lower memory overhead at scale.
...
ReLU activations ($18{,}219 \to 4{,}096 \to 1{,}024 \to 512$, total $\sim$80M
parameters, of which $\sim$75M reside in the first layer). Two design choices are
dictated by the structure of WJ and of the quadtree input, rather than by
convention.

\noindent\textbf{Global projection.}
WJ (Eq.~\ref{eq:wj}) is a \emph{global} statistic: its $\min$/$\max$ are summed
over all $D=18{,}219$ dimensions simultaneously and cannot be decomposed into
local sub-computations. The encoder must therefore preserve cross-dimension
co-occurrence structure. We enforce this by making the first layer a single
dense projection $\mathbb{R}^{18{,}219}\!\to\!\mathbb{R}^{4{,}096}$ that mixes
\emph{all} input dimensions in one matrix multiply, so every pair of quadtree
cells can interact. This is the critical distinction from sequence- or
patch-based encoders (e.g.\ Transformers), which decompose the input into local
token windows and aggregate them, destroying the long-range co-occurrence
statistics WJ depends on; such encoders collapse to $R@50<0.02$ on this task
regardless of the training loss~\cite{sampath25}.
...
We presented a WJ-native autoencoder compression framework for efficient polygon
similarity search. The key design choices --- probability simplex output constraint,
WJ triplet training with false-negative filtering, and decoder reconstruction ---
work together: our autoencoder achieves the highest Stage-1 recall of all
ANN-compatible methods and 78$\times$ higher throughput than brute-force ICWS,
with near-perfect recall after Stage-2 reranking.
The central empirical insight is that reconstruction and WJ triplet loss are
complementary only when false-negative filtering is active; without it, they
produce conflicting gradients and collapse. Future work will extend evaluation to
the full OSM Parks polygon corpus and explore 256- and 1024-dimensional
embedding variants to characterise the recall--dimensionality tradeoff.
